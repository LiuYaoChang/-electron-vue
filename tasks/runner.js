...
function run (command, color, name) {
  ...
  child.stdout.on('data', data => {
     ...
     */
    console.log('child.stdout.on data', data.toString().trim())
    if (/successfully/g.test(data.toString().trim().replace(/\n/g, '\n' + repeat(' ', command.length + 2))) && !isElectronOpen) {
...
run(`cross-env NODE_ENV=development PLAY_ENV=true webpack serve --config ./builds/webpack.demo.js --content-base app/dist`, YELLOW, 'webpack')
